...
class mangle:
	...
	def add_options(self, parser=None, usage=None, config=None):
		...
		parser.add_option('--smoothfunc',default='georgebazin',type='string',
...
files corresponding to each SN filter (In <filtpath>/<survey>.	If not provided, mangle tries <filter>.dat""",nargs=2)
	...
	def main(self,snfile,templatefile):
		...
		snid = sn.SNID
		if 'TELESCOPE' not in sn.__dict__.keys():
		...
		if sn.__dict__.has_key('PEAKMJD'): sn.tobs = sn.MJD - sn.PEAKMJD
		...
		if self.options.smoothlc and self.options.interplc:
			...
			sn.tobs = tobsout[:]
			sn.FLT = fltout[:]
			if sn.__dict__.has_key('PEAKMJD'): sn.MJD = sn.tobs + sn.PEAKMJD
		...
		if not self.options.smoothfluxfile:
			...
			print >> fout, '# filtfile zpt phase flux fluxerr'
			for flt in sn.FILTERS:
				if sn.__dict__.has_key('PEAKMJD'):
				...
				for t,fl,e in zip(tobs,flux,err):
					...
					if t > self.options.tobsrange[0] and t < self.options.tobsrange[1]:
						if usefiltfiles:
							print >> fout, '%s/%s/%s  %.4f	%.4f  %8.5e	 %8.5e'%(
								os.path.expandvars(self.options.filtpath),sn.TELESCOPE,
								self.options.filterfiles[1][self.options.filterfiles[0] == flt][0],
								27.5,t,fl,e)
						else:
							print >> fout, '%s/%s/%s.dat  %.4f	%.4f  %8.5e	 %8.5e'%(
								os.path.expandvars(self.options.filtpath),sn.TELESCOPE,flt,
								27.5,t,fl,e)
			fout.close()
...
### %i-%i-%i"""%(sn.SNID,tmplfile,time.localtime()[0],time.localtime()[1],time.localtime()[2])
		snt = txtobj(sedfile)
		...
		print >> fout, "NEPOCH: %i"%len(np.unique(sn.tobs))
		if sn.__dict__.has_key('SNTYPE'): sntype = sn.SNTYPE
		else: sntype = ''
		print >> fout, "SNTYPE: %s"%sntype
		for f,a in zip(sn.FILTERS,'abcdefghijklmnopqrstuvwxyz'):
			if usefiltfiles:
				print >> fout, "FILTER:	 %s	 %s/%s"%(
					a,self.options.filtpath,self.options.filterfiles[1][self.options.filterfiles[0] == f][0])
			else:
				print >> fout, "FILTER:	 %s	 %s/%s.dat"%(
					a,self.options.filtpath,f)
...
def smoothlc(sn,tobsrange=[-30,90],mkplot=False,workdir='workdir',addpts=True,smoothfunc=None,shockbreakout=False):
	from scipy.optimize import minimize
	import numpy as np

	if 'FAKEFLAG' not in sn.__dict__.keys():
...
#	 sn.FLUXCALERR[:] = 1e-2
	...
	for flt,i in zip(sn.FILTERS,range(len(sn.FILTERS))):
		icol = np.where((sn.tobs/(1+sn.z) >= tobsrange[0]-1) &
						(sn.tobs/(1+sn.z) <= tobsrange[1]+1) &
						(sn.FLT == flt))
	...
	if sn.__dict__.has_key('PEAKMJD'): sn.MJD = tsmooth + sn.PEAKMJD
...
#	 expterm = np.exp(-(t-tmax)/np.abs(tfall[0]))/(1+np.exp(-(t-tmax)/np.abs(trise[0])))
	for f,i in zip(np.unique(flt),range(filtlen)):
...
def doMangle(snid,z,niter=9,sntemp=None,
	...
	for i in range(niter):
		...
		snt.phase = pp[:]
		nphase += 2
		nflux = np.zeros([len(snt.wavelength)/(nphase-2),nphase])
		...
		snt.wavelength = snt.wavelength[0:len(snt.wavelength)/(nphase-2)]
		if mkplot and i == 0:
		...
		for j in range(len(nowphase)):
			for h in range(len(snt.wavelength)):
				print >> fout, '%s %s %s'%(nowphase[j],snt.wavelength[h],snt.flux[h,j])
		fout.close()
		if mkplot:
